...
bool flash_default_config(SerialPort *port) {
	...
	return flash_mem(port, data, sizeof(data), 0x3FC000);
}
	...
	if(currentArg >= argc) {
		if(developerMode) {
				printf( "Developer mode.\r\n"
						"Flashing:\r\n" \
						"0x00000 <- gamepad4.bin, using gamepad4.bin.prev as previously written data\r\n" \
			...
			printf( "No image file were specified. You can specify it in args by pairs\r\n" \
					...
					"0x00000 <- gamepad4.bin\r\n" \
					"0x3FC000 <- default configuration\r\n"
					);
		}
		char dh[] = "gamepad4.bin";
		int l = strlen(argv[0]);
		char df[l + sizeof(dh)];
		char *filename = dh;
		FILE *fl = fopen(dh, "rb");
		...
		isSuccess = flash_file(port, filename, 0x00000,
				developerMode ? (char*)"gamepad4.bin.prev" : NULL);
        ...
        char prev[256];
        strcpy(prev, argv[currentArg + 1]);
        strcat(prev, ".prev");
				isSuccess = flash_file(port, argv[currentArg + 1], address, prev);
			}
		}
	}
