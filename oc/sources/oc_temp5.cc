...
void render_voxel() {
  ...
  int32_t zd = dx;
	if (dx > dy) {
		e = dy - dx;
		for (int i = 0; i <= dx; i++) {
      rays[rc].rx = (((i - dx / 2) * 241) << 6) / dx;
      rc += gen_ray(&rays[rc], hx + 80, hy + 64, cols, cc);
			if (e >= 0) {
				e -= dx;
        hy += yinc;
        //rc += gen_ray(&rays[rc], hx + 80, hy + 64, cols, cc);
  ...
  printf("%d %d %d %d\n", rc, cc, rays[0].rx, rays[rc - 1].rx);
  //return;

  /*for (int x = 0; x < rc; x++) {
    uint8_t *fb = (uint8_t *)framebuffer;
    fb += rays[x].start;
    for (int ci = rays[x].cstart; ci > rays[x].cend; ci--) {
      uint32_t h = cols[ci].h;
      *fb = ((h & 16) ^ ((rays[x].rx & 32) >> 1)) ? 0xFF : 0;
      fb -= cols[ci].y * 160 + cols[ci].x;
    }
  }
  return;*/
  ...
  int32_t zz = 240 << 6;
  int32_t zz1 = -(rays[0].rx * 240) / 80;
  int32_t zz2 = (rays[rc - 1].rx * 240) / 80;
  ...
  for (int x = 0; x < rc; x++, lx += dx, ly += dy) {
    ...
    if (rdx > rdy) {
      ...
      for (int i = 0; i <= (rdx >> 15); i++) {
        ...
        int h = (v >> 8);
        h = ((((height - h) * testh) * divmap[z]) >> 16) + horizon;
        uint8_t c = v;
        if (h > rays[x].h)
          continue;
        if (h < rays[x].hend)
...
#ifndef USE_FLASH
  uint32_t cacheaddr = 0xFFFFFFFF;
  uint16_t cachedata[32];
#endif
  for (int32_t z = 5; z < 240; z++) {
    int32_t zz = z << 6;
    int32_t zz1 = -(rays[0].rx * z) / 80;
    int32_t zz2 = (rays[rc - 1].rx * z) / 80;
    int32_t lx = (-zz1 * cosa - zz * sina) >> 6;
    int32_t ly = (-zz * cosa + zz1 * sina) >> 6;
    int32_t rx = (zz2 * cosa - zz * sina) >> 6;
    int32_t ry = (-zz * cosa - zz2 * sina) >> 6;
    ...
    lx += (mx << 15) + 0x7FF8000;
    ly += (my << 15) + 0x7FF8000;
    for (uint32_t x = 0; x < rc; x++, lx += dx, ly += dy) {
      uint32_t j0 = ((ly >> 15) & 3);
      uint32_t k0 = ((lx >> 15) & 3);
      uint32_t j1 = (ly >> 17) & 0x3F;
      uint32_t k1 = (lx >> 17) & 0x3F;
      uint32_t v;
#ifdef  USE_FLASH
      uint32_t addr = 0x80000 + j1 * 512 * 4 + k1 * 32 + j0 * 8 + k0 * 2;
#ifdef  OCEMU
