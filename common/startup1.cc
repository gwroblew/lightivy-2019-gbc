...
void ROCODE sendwords(uint32_t *data, uint32_t size) {
	while(READ_PERI_REG(SPI_CMD(1))&SPI_USR);		//waiting for spi module available
...
extern void i2sStop();
extern void i2sInit(int rate, int lockBitcount);
extern unsigned int *i2sGetNextBuffer();

#ifndef BOOT_IMAGE
uint8_t ramblock1[RAM_MEM_BLOCK1_SIZE];
uint8_t *ramblock2;

void * ROCODE get_mem_block(uint32_t i) {
  if (i == 0)
    return ramblock1;
  return ramblock2;
}

uint32_t ROCODE get_mem_size(uint32_t i) {
  if (i == 0)
    return RAM_MEM_BLOCK1_SIZE;
  return RAM_MEM_BLOCK2_SIZE;
}
...
uint32_t ROCODE play_audio(uint32_t rate) {
  i2sInit(rate, 0);
  return 256;
}
...
void ROCODE log_line() {
  ...
  for (j = 0; j < 8; j++) {
    for (k = 0; k < 20; k++) {
      uint16_t c = 0xffff;
      int d = font8x8_basic[(unsigned char)logbuf[k] - 32][j];
...
int ROCODE read_dir(char *path, FILEINFO *filebuf, uint32_t *bufsize) {
  ...
  sd_mode();
	...
	if (res == FR_OK) {
      ...
      os_strncpy(filebuf[i].filename, fno.fname, FILEINFO_FILENAME_SIZE);
...
void boot_gpio_init() {
  ...
  //LCD_CS_SET();
    WRITE_PERI_REG(RTC_GPIO_OUT,
                   (READ_PERI_REG(RTC_GPIO_OUT) & (uint32)0xfffffffe) | 1);
  os_delay_us(1000);
}
...
void ROCODE hal_init()
{
  ...
  DEBUG("Reset: %d. Heap: %d\n", reset_info->reason, system_get_free_heap_size());
  printf("Reset: %d. Heap: %d\n", reset_info->reason, system_get_free_heap_size());
...
#ifndef BOOT_IMAGE
  memset(ramblock1, 0, RAM_MEM_BLOCK1_SIZE);
  ramblock2 = (uint8_t *)os_malloc_iram(RAM_MEM_BLOCK2_SIZE);
  memset(ramblock2, 0, RAM_MEM_BLOCK2_SIZE);
#endif
...
extern void ROCODE nosdk_init();

extern void ets_wdt_disable(void);
extern void Cache_Read_Enable(int a, int b, int c);
...
void nosdk_entry() {
  ...
  // and Speed QSPI: 0 = 40MHz, 1= 26MHz, 2 = 20MHz, ... = 80MHz
	...
	uint32 value = BIT(12); // 0x60000208 |= 0x1000
