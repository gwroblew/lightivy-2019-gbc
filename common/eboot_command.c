...
uint32_t ROCODE crc_update(uint32_t crc, const uint8_t *data, size_t length)
{
    uint32_t i;
    bool bit;
    uint8_t c;
...
uint32_t ROCODE eboot_command_calculate_crc32(const struct eboot_command* cmd)
{
    return crc_update(0xffffffff, (const uint8_t*) cmd, 
                      offsetof(struct eboot_command, crc32));
}

int ROCODE eboot_command_read(struct eboot_command* cmd)
{
...
void ROCODE eboot_command_write(struct eboot_command* cmd)
{
...
void ROCODE eboot_command_clear()
{
