...
#ifdef  BOOT_IMAGE
int __attribute__((section(".iram2.text"))) SPIEraseAreaEx(const uint32_t start, const uint32_t size)
#else
int __attribute__((section(".irom0.text"))) SPIEraseAreaEx(const uint32_t start, const uint32_t size)
#endif
{
